...
export class DailyTradeStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const riskParametersTable = new dynamodb.Table(this, 'RiskParametersTable', {
      partitionKey: { name: 'userId', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      removalPolicy: cdk.RemovalPolicy.RETAIN,
      pointInTimeRecovery: true,
    ...
    const localOpenSearchRole = new iam.Role(this, 'LocalOpenSearchAccessRole', {
    ...
    const openSearchSearchSlowLogGroup = new logs.LogGroup(this, 'OpenSearchSearchSlowLogGroup', {
      logGroupName: '/aws/opensearch/domains/daily-trade-knowledge-001/search-slow-logs',
      retention: logs.RetentionDays.ONE_WEEK,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });

    const openSearchIndexSlowLogGroup = new logs.LogGroup(this, 'OpenSearchIndexSlowLogGroup', {
      logGroupName: '/aws/opensearch/domains/daily-trade-knowledge-001/index-slow-logs',
      retention: logs.RetentionDays.ONE_WEEK,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });

    const openSearchErrorLogGroup = new logs.LogGroup(this, 'OpenSearchErrorLogGroup', {
      logGroupName: '/aws/opensearch/domains/daily-trade-knowledge-001/error-logs',
      retention: logs.RetentionDays.ONE_WEEK,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });
    ...
    const openSearchDomain = new opensearch.Domain(this, 'OpenSearchDomain', {
      ...
      accessPolicies: [
        new iam.PolicyStatement({
          ...
          actions: ['es:*'],
          resources: [`arn:aws:es:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:domain/daily-trade-knowledge-001/*`],
        }),
      ]
    });
    ...
    localOpenSearchRole.addToPolicy(new iam.PolicyStatement({
      effect: iam.Effect.ALLOW,
      actions: [
        'es:ESHttpGet',
        'es:ESHttpPut',
